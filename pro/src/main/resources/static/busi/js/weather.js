...
$(document).ready(function(){
    ...
    native.get(function(r){
        city = r.name;
        var Settings = '1,1,1,1,' + arrayThemes[randomTheme] + ','+city;
        alert(city);
    ...
    function ApplyData(d){
        ...
        // Central Info
        var currentTemp = d.item.condition.temp;
        var icon = WeatherIcon(d.item.condition.code);
        ...
        if( $atm.prop('checked') == true ){
            ...
            var pressure = d.atmosphere.pressure;
            var visib = d.atmosphere.visibility;
        ...
        if( $sun.prop('checked') == true ){
            ...
            var sunrise = d.astronomy.sunrise.replace(" am","").split(":");
            var sunset = d.astronomy.sunset.replace(" pm","").split(":");
            ...
            var totalHours = (parseInt(sunset[0]) + 12) - parseInt(sunrise[0]);
            var minDif,
        ...
        for(var item = 0; item < $10days.length; item++){
            var CurrentDay = d.item.forecast[item+1].day;
            var CurrentTemp = d.item.forecast[item+1].high;
            var CurrentTempLow = d.item.forecast[item+1].low;
            var CurrentIcon = WeatherIcon(d.item.forecast[item+1].code);
    ...
    function getWeather(loc){
        var querie = 'https://query.yahooapis.com/v1/public/yql?q=select units,astronomy,atmosphere,wind,location,item from weather.forecast where woeid in (select woeid from geo.places(1) where text="'+ loc +'") and u="c"&format=json';
    ...
    function LoadLocalStorage(){
        ...
        if (LocalSettings == null||$.trim(LocalSettings) == ''){
            var Settings = '1,1,1,1,' + arrayThemes[randomTheme] + ','+city;
            SettingsArray = Settings.split(',');
            getWeather(city);
            toLocalStorage();
    ...
    $('#settings').on('click','#update-button',function(e){
        ...
        if( temp_location == "" ){
            ...
            if( $rightMenu.hasClass('show') && (checkforblank == true ) ){
                alert(1);
